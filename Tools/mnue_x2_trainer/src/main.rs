...
const DEFAULT_OUTPUT_DIR: &str = "runs/mnue_x2_edges_300m";
const DEFAULT_NET_DIR: &str = "nets/mnue";
const DEFAULT_NET_ID: &str = "mnue_x2_edges_300m";
        ...
        while idx < args.len() {
            ...
            let (key, value) = if let Some((key, value)) = key_value.split_once('=') {
                ...
                let Some(value) = args.get(idx) else {
                    die_main(format!("missing value for --{key_value}"));
                };
            ...
            match key {
                "arch" => {
                    if value.trim() != "x2" {
                        die_main("--arch must be x2 for the legacy X2 trainer");
                    }
                }
                "data" | "train-data" => {
                    let value = value.trim();
                    if value.is_empty() {
                        die_main("--data must not be empty");
                    }
                    data.push(value.to_string());
                }
                "data-dir" => data_dirs.push(parse_main_path("data-dir", &value)),
                "chunk-shuffle-seed" => chunk_shuffle_seed = parse_main_u64(key, &value),
                "chunk-virtual-epochs" => chunk_virtual_epochs = parse_main_usize(key, &value),
                "chunk-sample" => chunk_sample = Some(parse_main_usize(key, &value)),
                "output-dir" => output_dir = parse_main_path(key, &value),
                "net-dir" => net_dir = parse_main_path(key, &value),
                "net-id" => net_id = parse_main_path(key, &value),
                _ => die_main(format!("unknown option --{key} for the legacy X2 trainer")),
            }
...
fn parse_main_path(option: &str, value: &str) -> String {
    let value = value.trim();
    if value.is_empty() {
        die_main(format!("--{option} must not be empty"));
    }
    value.to_string()
}

fn parse_main_u64(option: &str, value: &str) -> u64 {
    value
        .replace('_', "")
        .parse()
        .unwrap_or_else(|_| die_main(format!("--{option} must be a non-negative integer")))
}

fn parse_main_usize(option: &str, value: &str) -> usize {
    value
        .replace('_', "")
        .parse()
        .unwrap_or_else(|_| die_main(format!("--{option} must be a positive integer")))
}
...
fn main() {
    ...
    let data_schedule = build_data_schedule(config.data_schedule_options())
        .unwrap_or_else(|error| die_main(error.to_string()));
    println!(
