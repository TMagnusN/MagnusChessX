...
use crate::config::{DEFAULT_BATCH_SIZE, DEFAULT_NET, DEFAULT_OUTPUT_DIR};
use mnue_data_schedule::{DEFAULT_CHUNK_SHUFFLE_SEED, DEFAULT_CHUNK_VIRTUAL_EPOCHS};
        ...
        S: Into<String>,
    {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--chunk-shuffle-seed" => {
                    cli.chunk_shuffle_seed = next_value(&mut args, &arg)?
                        .parse()
                        .map_err(|_| "invalid --chunk-shuffle-seed".to_string())?;
                }
                "--chunk-virtual-epochs" => {
                    cli.chunk_virtual_epochs = next_value(&mut args, &arg)?
                        .parse()
