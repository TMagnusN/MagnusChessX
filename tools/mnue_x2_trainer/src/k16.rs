...
use bullet_lib::value::loader::{DataLoader as DirectDataLoader, DirectSequentialDataLoader};
use bullet_trainer::{
    Trainer,
...
const DEFAULT_DATA: &str = "D:/NNUE/data/train_3b_shuffled.data";
const DEFAULT_OUTPUT_DIR: &str = "D:/NNUE/runs/mnue_x2_k16_pawn_q8_a384";
const DEFAULT_EXPORT: &str = "D:/NNUE/nets/mnue/mnue_x2_k16_pawn_q8_a384.mnue";
...
impl Cli {
    ...
    const fn help() -> &'static str {
        "MNUE-X2 trainer\n\
         ...
         --data PATH               training data path; repeatable\n\
...
fn inspect_data_stats(paths: &[PathBuf], limit: u64) -> io::Result<DataStats> {
...
fn read_samples<F>(paths: &[PathBuf], limit: u64, mut accept: F) -> io::Result<usize>
where
    F: FnMut(Sample) -> bool,
{
...
impl DataLoader for MnueK16DataLoader {
    fn map_batches<F: FnMut(PreparedBatchHost) -> bool>(
        ...
        direct.map_chunks(0, |chunk: &[ChessBoard]| {
...
fn run_training(cli: Cli) -> io::Result<()> {
    ...
    let accepted = read_samples(&cli.data, inspect_limit, |sample| {
    ...
    if cli.data_stats_limit > 0 {
        let stats = inspect_data_stats(&cli.data, cli.data_stats_limit)?;
        stats.print();
    }
