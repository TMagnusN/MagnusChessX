...
Run:
    cargo run --manifest-path tools/mnue_p2_trainer/Cargo.toml --release --features cuda -- \
        --arch p2-k32 --name mnue_p2k32_100m_5sb --positions 100000000 \
...
// Keep training data fixed here; this script deliberately has no --train-data flag.
const TRAIN_DATA: &str = "D:/NNUE/data/merged.data";
const RUNS_DIR: &str = "D:/NNUE/runs";
...
impl Config {
    fn from_args() -> Self {
        ...
        let mut net_dir = NET_DIR.to_string();
        ...
        let output_dir = output_dir.unwrap_or_else(|| format!("{RUNS_DIR}/{name}"));
    ...
    fn print_summary<S: P2Spec>(&self) {
    ...
    println!(
        "Usage: cargo run --manifest-path tools/mnue_p2_trainer/Cargo.toml --release --features cuda -- [options]\n\
         ...
         Options:\n\
           ...
           --output-dir PATH           Default: {RUNS_DIR}/NAME\n\
           --net-dir PATH              Default: {NET_DIR}"
    );
}
...
fn run_training<S: P2Spec>(config: Config) {
    ...
    let data_loader = loader::DirectSequentialDataLoader::new(&[TRAIN_DATA]);
